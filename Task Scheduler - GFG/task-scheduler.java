...
class Solution {
    static int leastInterval(int N, int K, char tasks[]) {
        // code here      // code here
         int[] freq = new int[26];  // Frequency array to store count of each task
        for (char task : tasks) {
            freq[task - 'A']++;
        }
        ...
        int idleTime = (maxFreq - 1) * K;  // Calculate total idle time

        for (int i = 24; i >= 0 && freq[i] > 0; i--) {
            idleTime -= Math.min(maxFreq - 1, freq[i]);  // Deduct the idle time based on tasks with same frequency
        }

        idleTime = Math.max(0, idleTime);  // Idle time cannot be negative

        return N + idleTime;  // Total units of time required
    }
}
